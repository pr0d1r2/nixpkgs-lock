#!/usr/bin/env bash
#
# propagate.sh — Propagate nixpkgs-lock pin to downstream nix-* repositories
#
...
#
# Usage:
#   ./scripts/propagate.sh              # all public repos (except nixpkgs-lock)
#   ./scripts/propagate.sh 'nix-*'      # only nix-* repos
#   ./scripts/propagate.sh 'nix-lefthook-*'  # subset
#
set -euo pipefail
